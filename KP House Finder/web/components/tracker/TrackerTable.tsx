        ...
        <table className="w-full border-collapse text-xs" style={{ minWidth: "980px" }}>
          <thead>
            <tr style={{ background: "var(--bg)" }}>
              {["Listing", "Name", "Location", "Date", "Contact", "Notify before", "Notes", ""].map((c, i) => (
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={8} className="px-2 py-8 text-center" style={{ color: "var(--muted)" }}>No rows yet — click “+ Add row”.</td></tr>
            )}
            {rows.map((r) => {
